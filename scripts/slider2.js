...
left2.addEventListener('mousedown', function(down) {
	...
	let interval = setInterval(shiftLeft2, 500);
...
right2.addEventListener('mousedown', function(down) {
	if (down.button !== 0) return;
	let interval2 = setInterval(shiftRight2, 500);
...
//SLIDER
const imgWidth2 = 290; // ширина картинки+ отступ
const amountImgForShow2 = 4; // сколько картинок показываем
let shiftX_2 = imgWidth2; // на сколько сдвинуть по оси x
let shiftX_22 = 0;
let counter2 = 0;
let offset2 = 0;
// if(blockId2=="slide1"){ images = document.querySelectorAll('#slide1 img');}
// if(blockId2=="slide2"){ images = document.querySelectorAll('#slide2 img');}
images2 = document.querySelectorAll(`#${blockId2} .product__cart`);
